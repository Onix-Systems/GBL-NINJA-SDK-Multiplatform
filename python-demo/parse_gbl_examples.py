        ...
        if len(extra_data) <= 64:  # Show if not too long
            try:
                ...
                if text.isprintable():
                    ...
                    print(f"     Content (hex): {extra_data.hex()}")
            except:
                print(f"     Content (hex): {extra_data.hex()}")
...
def analyze_metadata_tag(tag):
    ...
    try:
        ...
        if text_content.isprintable():
            ...
            print(f"   Content (text): {display_text}")
        else:
...
def analyze_prog_lz4_tag(tag):
    ...
    if len(tag.tag_data) >= 8:
        try:
            ...
            ratio = (len(compressed_data) / decomp_size) * 100 if decomp_size > 0 else 0
...
def verify_file_integrity(tags, original_data):
    ...
    print(f"File CRC32: 0x{end_tag.gbl_crc:08X}")
    ...
    try:
        try:
            ...
            print("⚠️  CRC verification function not available")
            return True
...
def extract_firmware_data(filename):
    ...
    try:
        ...
        for i, prog_tag in enumerate(prog_tags):
            output_filename = f"{base_name}_prog_section_{i}.bin"
...
def create_sample_files_if_missing():
    from create_gbl_examples import create_simple_gbl, create_advanced_gbl, create_compressed_gbl

    files_to_check = [
        ("simple_firmware.gbl", create_simple_gbl),
        ("advanced_firmware.gbl", create_advanced_gbl),
        ("compressed_firmware.gbl", create_compressed_gbl)
    ]

    created_any = False
    for filename, create_func in files_to_check:
        if not os.path.exists(filename):
...
def main():
    ...
    print("=" * 50)
    print("Using gbl-ninja-python package\n")
    ...
    try:
        ...
        if not parsed_files:
            ...
            print("Run create_gbl_examples.py first to create test files.")

    except ImportError as e:
        print(f"❌ Import error: {e}")
        print("\nPlease install the required package:")
        print("  pip install gbl-ninja-python")
