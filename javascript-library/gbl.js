...
class TagContainer {
    ...
    create() {
        try {
            ...
            this.content.add(headerTag);

            const endTag = this.createEndTag();
            this.content.add(endTag);
    ...
    remove(tag) {
        try {
            ...
            const removed = this.content.delete(tag);
            if (!removed) {
                return new ContainerResult.Error(
    ...
    build() {
        try {
            ...
            const sortedTags = [];
            const contentArray = Array.from(this.content);

            const headerTag = contentArray.find(tag => tag.tagType.name === 'HEADER_V3');
            if (headerTag) {
            ...
            const endTag = contentArray.find(tag => tag.tagType.name === 'END');
            if (endTag) {
    ...
    hasTag(tagTypeName) {
        ...
        return Array.from(this.content).some(tag => tag.tagType.name === tagTypeName);
    }

    getTag(tagTypeName) {
        if (!this.isCreated) return null;
        return Array.from(this.content).find(tag => tag.tagType.name === tagTypeName);
    }

    isEmpty() {
        return !this.isCreated || this.content.size === TagContainer.PROTECTED_TAG_TYPES.size;
    }

    size() {
        return !this.isCreated ? 0 : this.content.size;
    }

    getTagTypes() {
        if (!this.isCreated) return new Set();
        return new Set(Array.from(this.content).map(tag => tag.tagType.name));
    }
    ...
    clear() {
        try {
            ...
            const toRemove = Array.from(this.content).filter(tag => !this.isProtectedTag(tag));
            toRemove.forEach(tag => this.content.delete(tag));
...
function createEmptyGblFile() {
    const emptyGblData = new Uint8Array([
        0xEB, 0x17, 0xA6, 0x03, 0x08, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00,
        0xF4, 0x0A, 0x0A, 0xF4, 0x0D, 0x00, 0x00, 0x00,
        0x20, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x36,
        0xFD, 0x03, 0x03, 0xFD, 0x08, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0xFC, 0x04, 0x04, 0xFC, 0x04, 0x00, 0x00, 0x00,
        0x8E, 0x7B, 0x7A, 0x18
    ]);
...
function testGblParsing() {
    console.log("=== Testing GBL Parser ===");
    
    const gbl = new Gbl();
    const emptyGblData = createEmptyGblFile();
    
    console.log("Created empty GBL file with", emptyGblData.length, "bytes");
    
    const parseResult = gbl.parseByteArray(emptyGblData);
    
    if (parseResult.type === 'Success') {
        const tags = parseResult.resultList;
        console.log(`Successfully parsed GBL file with ${tags.length} tags:`);
        
        const tagTypes = [];
        tags.forEach((tag, index) => {
            const tagName = tag.tagType.name;
            tagTypes.push(tagName.toLowerCase());
            console.log(`Tag ${index + 1}: ${tagName}`);
        });
        
        const expectedTags = ['header', 'application', 'erase_prog', 'end'];
        const actualTags = tagTypes.map(t => t.replace('_v3', '').replace('eraseprog', 'erase_prog'));
        
        console.log("Expected tags:", expectedTags);
        console.log("Actual tags:", actualTags);
        
        const hasAllExpected = expectedTags.every(expected => 
            actualTags.includes(expected)
        );
        
        if (hasAllExpected && actualTags.length === expectedTags.length) {
            console.log("✅ Test PASSED: Found all expected tags");
        } else {
            console.log("❌ Test FAILED: Missing or incorrect tags");
        }
    } else {
        console.log("❌ Test FAILED: Parse error:", parseResult.error);
    }
}
